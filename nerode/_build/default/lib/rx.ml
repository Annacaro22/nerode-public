...
let rx_0 = of_word (Word.of_intlist [0])

let rx_1 = of_word (Word.of_intlist [1])



let rec generate_regex (curr : t) (choices : (int*int) list)=
  match choices with
  | (action, digit) :: t ->
    (match action with
      |1 -> let digitrex = (match digit with
            ...
            |2 -> rx_1
            |_ -> rx_empty) in
            generate_regex (seq [curr; digitrex]) t
      |2 -> let digitrex = (match digit with
            |1 -> rx_0
            |2 -> rx_1
            |_ -> rx_empty) in
            generate_regex(union [curr; digitrex]) t
      |3 -> generate_regex(star curr) t
      |_ -> curr)
  | _ -> curr

(*this but replace randoms with quickcheck generators
