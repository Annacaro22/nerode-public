...
let kv_from_rx r () =
  ...
  let kv_from_rx_est r () =
    let alpha = Alphabet.intalph 2 in
    let teacher = Parser.parse_string r
                  |> Dfa.of_rx alpha
                  |> TeacherEstimate.make in
    let d = KVD_Est.learn alpha teacher in
    Dfa.print d;
    Printf.printf "Dfa size: %d\n%!" (Dfa.size d);
    Printf.printf "%s\n%!" (Dfa.to_rx d |> Rx.to_string alpha)
...
let learn_from_sep_cfg_rx (c : Pda.cfg) r () =
  let alpha = Alphabet.intalph 2 in
  let cfg : Pda.cfg = c in
  let rx = Parser.parse_string r in
  let teacher = TeacherCFG.make alpha cfg rx in
  (*automatically use three optimizations from paper in this setting*)
  let () = CliOpt.set_pq true in
  let () = CliOpt.set_unsat_cores true in
  let () = CliOpt.set_ge true in
  let () = Solver.produce_unsats () in
  let d = PDA.learn alpha teacher in
  Dfa.print d;
  Printf.printf "Dfa size: %d\n%!" (Dfa.size d);
  Printf.printf "%s\n%!" (Dfa.to_rx d |> Rx.to_string alpha)
...
let kvrx_est_cmd () =
  let open Stdlib.Arg in
  let rx = ref "" in
  let usage = "Run KV using an estimation teacher built from a Rx\n  kv-est <rx>" in
  parse [] (fun r -> rx := r) usage;
  kv_from_rx_est !rx ()
...
let lsb_sep_cmd () =
  ...
  let lsb_sep_cfg () =
    ...
    let neg = ref "" in
    let read_sets set =
      if String.is_empty !nonterms then
        nonterms := set
      else if String.is_empty !terms then
        terms := set
      else if String.is_empty !prods then
        prods := set
      else if String.is_empty !neg then
        neg := set in
    let usage = "Run the L* with Blanks on context free and regular languages\n  lsb-sep-cfg [L+] [L-]" in
    Stdlib.Arg.parse [] read_sets usage;
    let cfg = Pda.input_to_cfg (!nonterms) (!terms) ([!prods]) in
    learn_from_sep_cfg_rx cfg !neg ()
...
let () =
  let bad_cmd_str = "Invalid subcommand, valid are: lsblanks, lsblanks-dir, lsblanks-sep, lsblanks-cfg, kv, kv-est." in
